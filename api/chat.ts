...
export default async function handler(req: any, res: any) {
  ...
  if (req.method !== "POST") {
    res.status(405).json({ error: `Method ${req.method} not allowed. Please use POST.` });
    return;
  }
  ...
  try {
    ...
    // Safety Filter 1: Medical queries guard
    ...
    if (!apiKey) {
      res.status(500).json({
        error: "La clave GEMINI_API_KEY no está configurada en las variables de entorno de Vercel."
      });
      return;
    }
...
Ayúdale con dudas específicas de esta receta usando la información proporcionada. Si pregunta por sustitutos de ingredientes específicos para esta preparación, guíale con alternativas válidas en la cocina sin gluten (como usar harina de arroz, almidón de yuca/tapioca, fécula de maíz, harina de almendras, psyllium, etc.) de manera comprensible.`;
    ...
    res.status(200).json({ text: botResponseText });
  } catch (error: any) {
    console.error("Error in serverless API /api/chat:", error);
